...
def ejecutarOpcion(opcion):
    ...
    if opcion == 1:
        ...
        try:
            if len(cursos) > 0:
                curso = function.pedirDatosActualizacion(cursos)
                if curso:
        ...
        try:
            cursos = dao.listaDeCursos()
            if len(cursos) > 0:
                codigoEliminar = function.pedirDatosEliminacion(cursos)
